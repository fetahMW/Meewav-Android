...
| --- | --- | --- |
| La Scène (`VID-01` à `VID-10`) | Le catalogue publié Supabase, la fiche, le suivi et les interactions restent reliés. Le catalogue charge les pages suivantes de 48 médias à la demande ou au défilement ; une vidéo ouverte par lien est chargée séparément. La recherche serveur `scene_search_catalog_v1` couvre aussi les artistes, villes, types et hashtags, avec pagination, sans exposer la caméra secondaire comme publication indépendante. Les murs, recommandations, notifications, dates et libellés de publication ne puisent plus dans les vidéos figées en mode réel. Les pays filtrables viennent du catalogue reçu. La TV, fondée aujourd'hui sur une grille de démonstration, est réservée à la démo. | Les filtres avancés continuent d'opérer sur les pages chargées. Reprise/entretien des uploads et URL signées, modération, synchronisation inter-appareils des listes/historique, vraie programmation TV, droits/replays, sous-titres, analytics et partage public restent ouverts. |
...
### Suite de La Scène — recherche et publication multicam
