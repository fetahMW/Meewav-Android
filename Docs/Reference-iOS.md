...
- Les 28 avatars, leur ordre, leurs descriptions et leurs identifiants iOS sont conservés. Le rail tactile utilise Compose et les PNG HD ; les personnages centraux et les voisins restent opaques au repos, seuls les éléments quittant les bords s'effacent. Le plateau, les projecteurs, leur variation d'intensité et la chute de confirmation sont adaptés de l'iOS (voir ci-dessous).
...
L’intensité descend à 0,14 pendant le geste, remonte à l’arrêt puis se stabilise, selon la mécanique iOS. Le rendu des surfaces et faisceaux est mis en cache à la résolution de l’écran ; l’animation ne reconstruit pas ces textures à chaque image. Ce mécanisme n’est pas une mesure de fréquence d’affichage. Les identifiants backend et l’aperçu sans compte Android sont conservés. Ni l’authentification sociale iOS ni de nouvelles fonctions backend ne sont ajoutées par ce portage visuel.

Après retour sur téléphone, les images sont rendues hors du viewport du pager pour éviter la découpe latérale. Les extrémités sont abaissées de 6 dp progressivement et les quatre spots sont placés sur l’ellipse. La confirmation reprend les phases de `SaturnAvatarSlotView.swift` : impulsion, projection latérale puis chute accélérée en 0,92 seconde, avec un anneau de lumière et un léger mouvement de la vitre. Le personnage central reste en place ; après 0,1 seconde supplémentaire, le parcours normal passe à Compte. La disparition de l’écran annule la coroutine de confirmation. Le flou SwiftUI des personnages en chute n’est pas reproduit.

Le sélecteur, initialement paginé, devient à la demande de l’utilisateur un grand `ModalBottomSheet` sombre avec les 28 vignettes dans une grille à trois colonnes. Seule cette grille défile verticalement ; l’écran Avatar reste fixe. Choisir une vignette recentre le rail puis ferme le panneau.
