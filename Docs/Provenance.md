...
Les modules sous `app/src/main/globe-source/vendor/globe-vinyle` et `vendor/meewav-vinyl` ont été initialement copiés sans transformation depuis le Web. Le lot de [navigation tactile Android](Globe/Navigation-tactile.md) adapte ensuite six sources de navigation ; le manifeste conserve les empreintes originales, sans modification des ressources ni des licences. La texture `earth_specular.jpg` provient de `vendor/globe-vinyle/assets/ui/images/earth_specular.jpg`. Ces chemins identifient les ressources du projet ; ils ne constituent ni une nouvelle attribution d’auteur ni une licence publique des éléments Meewav.
