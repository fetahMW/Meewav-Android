...
function triggerDownload(fileName: string, source: Blob | string) {
  saveAttachment(fileName, source);
}
...
export default function MessageWorkspace({
  ...
  const draftConversation = useRef<string | null>(null);
  ...
  useEffect(() => {
    const compose = () => setShowNewConversation(true);
    window.addEventListener('meewav:messaging-compose', compose);
    return () => window.removeEventListener('meewav:messaging-compose', compose);
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    if (draftConversation.current === id) return;
    if (draftConversation.current) mobileDrafts.current.set(draftConversation.current, composerValue.current);
    draftConversation.current = id;
    setComposer(id ? mobileDrafts.current.get(id) ?? '' : '');
  ...
  // Mobile opens on the inbox. Do not mark the first conversation read until
  // the user explicitly opens it on the full-screen conversation surface.
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new Event('meewav:messaging-detail'));
    if (liveController) {
      onRequestConsumed?.();
      return;
    }
  ...
  useEffect(() => {
    ...
    window.addEventListener('meewav:messaging-list', stop);
    return () => {
      window.removeEventListener('meewav:messaging-suspend', stop);
      window.removeEventListener('meewav:messaging-list', stop);
    };
  }, [recordedVoice]);
