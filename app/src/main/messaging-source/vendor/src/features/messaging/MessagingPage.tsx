...
export default function MessagingPage() {
  ...
  const explicitDemoTarget = routeState.mode === "demo" && Boolean(routeState.mockArtistId);
  ...
  useEffect(() => {
    const create = (event: Event) => {
      if ((event as CustomEvent).detail === 'groups') setCreateGroupSignal(value => value + 1);
      else setCreateProjectRequest({ token: Date.now(), groupId: '', name: '' });
    };
    window.addEventListener('meewav:messaging-new-space', create);
    return () => window.removeEventListener('meewav:messaging-new-space', create);
  }, []);
