...
type Contact={id:string;name:string;avatar?:string;kind?:'audio'|'video'};
type Call={kind?:'audio'|'video';id:string;conversationId:string;peerId:string;peerName:string;incoming:boolean;status:'ringing'|'accepted'|'ended'|'declined'|'missed'};
...
export function openAudioCall(contact:Contact){openVideoCall({...contact,kind:'audio'});}
...
export default function VideoCalls(){
  ...
  const cameraPreview=async(contact:Contact)=>{
    ...
    setMuted(false);setCameraOff(contact.kind==='audio');setElapsed(0);setScreen({contact,phase:'prepare'});
    try{
      const stream=await navigator.mediaDevices.getUserMedia({video:contact.kind==='audio'?false:{facingMode:'user',width:{ideal:1280},height:{ideal:720}},audio:{echoCancellation:true,noiseSuppression:true}});
      ...
      setScreen({contact,phase:'error',message:error instanceof DOMException&&error.name==='NotAllowedError'
        ?(contact.kind==='audio'?'Autorise le micro dans les paramètres de Meewav, puis réessaie.':'Autorise la caméra et le micro dans les paramètres de Meewav, puis réessaie.')
        :'La caméra ou le micro est indisponible. Ferme les autres applications qui les utilisent, puis réessaie.'});
    }
  };
  useEffect(()=>{
    if(!preview.current || !local.current || !preview.current.getVideoTracks().length)return;
  ...
  const connect=async(value:Call)=>{
    ...
    try{
      ...
      const rtc=new VideoTransport(value.id,local.current,remote.current,
        ...
        resume=>{if(generation===epoch.current)setResumeAudio(()=>resume);},!muted,value.kind==='audio'?false:!cameraOff);
      transport.current=rtc;await rtc.connect();
    }catch(error){console.warn('[MeeWav call]',JSON.stringify({stage:'connect',name:error instanceof Error?error.name:'UnknownError'}));if(generation===epoch.current)problem('Impossible d’établir la connexion d’appel. Tu peux rappeler.');}
  };
  const consume=(value:Call|null)=>{
    ...
    if(!call.current && value.incoming){
      ...
      call.current=value;setMuted(false);setCameraOff(value.kind==='audio');setScreen({contact:{id:value.conversationId,name:value.peerName,kind:value.kind},phase:'incoming'});
  ...
  useEffect(()=>{
    ...
    const timeout=window.setTimeout(()=>functions.current.finish('La connexion d’appel n’a pas abouti. Réessaie.'),30_000);
  ...
  const accept=async()=>{
    ...
    try{
      window.dispatchEvent(new Event('meewav:messaging-suspend'));
      const media=await navigator.mediaDevices.getUserMedia({audio:true,video:value.kind==='audio'?false:{facingMode:'user'}});
  ...
  const audioOnly=screen.contact.kind==='audio';
  const live=screen.phase==='connected'||screen.phase==='connecting';
  const hasMedia=live||screen.phase==='preview'||screen.phase==='ringing';
  const status=screen.message||({prepare:audioOnly?'Ouverture du micro…':'Ouverture de la caméra…',preview:previewEnabled()?(audioOnly?'Aperçu audio · Aucun appel envoyé':'Aperçu caméra · Aucun appel envoyé'):'Prêt à appeler',incoming:audioOnly?'Appel audio entrant':'Appel vidéo entrant',ringing:'Appel en cours…',connecting:'Connexion…',connected:`${Math.floor(elapsed/60)}:${String(elapsed%60).padStart(2,'0')}`,ended:'Appel terminé',error:'Appel indisponible'}[screen.phase]);
  return createPortal(<section className={`mw-video-call is-${screen.phase}${audioOnly ? " is-audio" : ""}`} role="dialog" aria-modal="true" aria-label={`Appel ${audioOnly?"audio":"vidéo"} avec ${screen.contact.name}`}>
    <div ref={remote} className="mw-video-call__remote"/>
    <header><span>{audioOnly?<Phone size={18}/>:<Video size={18}/>} Appel {audioOnly?"audio":"vidéo"}</span><button aria-label="Fermer l’appel" onClick={()=>finish()}><X/></button></header>
    ...
    <footer>
      {hasMedia&&<div className="mw-video-call__controls">
        ...
        {!audioOnly&&<button disabled={busy||screen.phase==='connecting'} aria-label={cameraOff?'Activer la caméra':'Couper la caméra'} aria-pressed={cameraOff} onClick={()=>void control(async()=>{
        ...
        {live&&!audioOnly&&<button disabled={busy||cameraOff||screen.phase==='connecting'} aria-label="Changer de caméra" onClick={()=>void control(async()=>{await transport.current?.flip();})}><SwitchCamera/><span>Retourner</span></button>}
      </div>}
      {screen.phase==='incoming'?<div className="mw-video-call__answer"><button className="is-hangup" onClick={decline}><PhoneOff/> Refuser</button><button className="is-primary" disabled={busy} onClick={()=>void accept()}>{audioOnly?<Phone/>:<Video/>} {busy?'Autorisation…':'Accepter'}</button></div>
