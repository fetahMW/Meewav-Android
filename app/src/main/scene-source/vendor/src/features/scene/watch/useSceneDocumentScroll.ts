...
export function useSceneDocumentScroll(entryKey: string, enabled: boolean) {
 useLayoutEffect(() => {
  ...
  scroller.addEventListener('scroll', save, { passive: true }); document.addEventListener('pointerdown', save, true);
  return () => { cancelAnimationFrame(frame); scroller.removeEventListener('scroll', save); document.removeEventListener('pointerdown', save, true); history.scrollRestoration = previous; };
 }, [entryKey, enabled]);
}
