...
function SceneWorkspace() {
  ...
  const scrollHomeToTop = useCallback(() => {
    window.requestAnimationFrame(() => {
      if (document.querySelector(".scene-page.is-document")) document.querySelector<HTMLElement>(".scene-page")?.scrollTo({ top: 0, behavior: "smooth" });
  ...
  const openVideo = useCallback((item: VideoItem) => {
    ...
    const sceneScrollTop = document.querySelector<HTMLElement>(".scene-page")?.scrollTop || document.querySelector<HTMLElement>(".shorts-home")?.scrollTop || 0;
  ...
  const closeVideo = useCallback(() => {
    ...
    if (returnScrollTop !== null) {
      window.requestAnimationFrame(() => {
        window.requestAnimationFrame(() => {
          document.querySelector<HTMLElement>(".scene-page")?.scrollTo({ top: returnScrollTop, behavior: "instant" });
  ...
  useSceneDocumentScroll(`${location.key}:${location.pathname}${location.search}`, true);
    ...
    <main className={`shorts-page scene-page has-unified-header${activeTab !== "tv" || watchRoute ? " is-document" : ""}${watchRoute ? " has-watch-page" : ""}${isCreatorStudioRoute ? " is-creator-studio" : ""}${showBrowseNavigation ? ` has-browse-nav${browseMenuOpen ? "" : " is-browse-collapsed"}` : ""}`} aria-label={SCENE_NAME}>
      ...
      <div className="shorts-shell">
        ...
        <div className="mobile-scene-tools">          {!showBrowseNavigation && <button ref={watchMenuButtonRef} className="scene-browse-toggle" aria-label={watchMenuOpen ? "Fermer le menu vidéo" : "Ouvrir le menu vidéo"} aria-expanded={watchMenuOpen} aria-controls={watchMenuOpen ? "scene-browse-nav" : undefined} onClick={() => setWatchMenuKey(watchMenuOpen ? null : location.key)}><Menu aria-hidden="true" /></button>}
          ...
          <SceneSearch query={searchQuery} items={allVideos} onSearch={(query) => navigate(`${getScenePathForTab("explore")}${query ? `?q=${encodeURIComponent(query)}` : ""}`)} onFilters={openFilters} filtersOpen={filtersOpen} filterCount={activeFilterCount} inputRef={searchInputRef} filterRef={filterTriggerRef} />          <div className="shorts-topbar__actions">
