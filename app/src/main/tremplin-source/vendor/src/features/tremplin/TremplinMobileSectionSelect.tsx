...
export default function TremplinMobileSectionSelect<T extends string>({ value, options, onChange }: {
  value: T; options: readonly (readonly [T, string])[]; onChange: (value: T) => void;
}) {
  ...
  return <div className="mobile-tremplin-section" ref={root}>
    <button ref={trigger} type="button" aria-expanded={open} aria-controls={listId} aria-label={`Contenu de Mes artistes : ${options.find(([id]) => id === value)?.[1] ?? 'Aperçu'}`} onClick={() => setOpen(!open)}>
      <span>{options.find(([id]) => id === value)?.[1] ?? 'Aperçu'}</span><ChevronDown size={18} style={{ transform: open ? 'rotate(180deg)' : undefined }} />
    </button>
    {open && <div id={listId} role="navigation" aria-label="Contenu de Mes artistes">{options.map(([id, label]) => <button key={id} type="button" aria-current={value === id ? 'page' : undefined}
