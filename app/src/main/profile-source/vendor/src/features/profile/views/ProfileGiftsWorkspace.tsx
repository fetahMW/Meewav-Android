...
function GiftCatalog({
  ...
  return <div className="profile-gift-catalog">{giftCatalog.map((item) => {
    ...
    // Selecting prepares a local draft; inventory is checked by the Room at distribution.
    ...
    const availability = inventoryStatus === "ready"
      ...
      : inventoryStatus === "loading"
        ? "Vérification…"
        : "Brouillon possible";
    return <button key={item.name} type="button" className={`profile-gift-catalog-card is-${item.visual} ${isSelected ? "is-selected" : ""} ${item.gold ? "is-gold" : ""} ${canSelect ? "" : "is-unavailable"}`} style={{ "--gift-tone": item.tone } as React.CSSProperties} aria-pressed={isSelected} onClick={() => canSelect && onSelect(item.name)}><GiftObject item={item} /><div className="profile-gift-catalog-card__copy"><span className="profile-gift-catalog-card__meta"><small>{item.rarity}</small><em className={canSelect ? "is-available" : "is-unavailable"}>{availability}</em></span><strong>{item.name}</strong><p>{item.detail}</p></div><i className="profile-gift-catalog-card__select">{isSelected ? <Check size={15} /> : showAdd && canSelect ? <Plus size={15} /> : null}</i></button>;
  })}</div>;
}
...
export default function ProfileGiftsWorkspace({ storageScope, onBack, onDone }: ProfileGiftsWorkspaceProps) {
  const storageKey = storageScope ? `${STORAGE_KEY}:${storageScope}` : isProfileLocalPreviewEnabled() ? `${STORAGE_KEY}:android-preview` : null;
  ...
  const saveGift = () => {
    ...
    // Saving a draft never reserves or debits a gift. Distribution remains server-controlled in Rooms.
  ...
  return <section className="profile-studio-workspace profile-studio-mode-shell is-gifts profile-gifts-workspace" style={{ "--studio-flow-accent": "#d946ef" } as React.CSSProperties} aria-label="Cadeaux">
    ...
    {screen === "overview" && <div className="profile-studio-mode-overview">
      <header className="profile-studio-mode-heading profile-gifts-overview-hero"><button className="profile-studio-workspace__back" type="button" onClick={goBack}><ArrowLeft size={16} /> Studio</button><span className="profile-gifts-overview-hero__divider" aria-hidden="true" /><div className="profile-gifts-overview-hero__copy"><span className="profile-kicker"><Gift size={14} /> Fans & communauté · Cadeaux</span><h3>Tes cadeaux</h3><p>Prépare tes brouillons privés ici. La distribution se fait ensuite en Room.</p></div><button type="button" className="profile-primary-button" onClick={() => startNew()}><Plus size={17} /> Préparer un cadeau</button></header>
      <details className="profile-gifts-inventory-disclosure"><summary><PackageOpen size={16} /> Mon inventaire Room<ChevronRight size={16} /></summary><ProfileGiftInventoryPanel inventory={giftInventory} status={giftInventoryStatus} onRetry={() => setGiftInventoryRefresh((current) => current + 1)} /></details>
      <ProfileMenuSelect label="Afficher les cadeaux" variant="filter" value={filter} options={[{ value: "Inventaire", label: "Catalogue", count: giftCatalog.length }, { value: "Brouillons", label: "Brouillons", count: entries.filter(entry => entry.status !== "Annulé").length }, { value: "Archivés", label: "Archivés", count: entries.filter(entry => entry.status === "Annulé").length }]} onChange={setFilter} />
      {filter === "Inventaire" && <section className="profile-gifts-catalog-section" aria-label="Catalogue de cadeaux à préparer"><header><div><span className="profile-kicker"><Sparkles size={14} /> Catalogue</span><h4>Choisir un cadeau</h4></div>{catalogChoice && <button type="button" className="is-primary" onClick={() => startNew(catalogChoice, 1)}>Préparer <ArrowRight size={15} /></button>}</header><GiftCatalog inventory={giftInventory} inventoryStatus={giftInventoryStatus} selected={catalogChoice} showAdd onSelect={(name) => setCatalogChoice((current) => current === name ? null : name)} /></section>}
    ...
    {screen === "editor" && <div className="profile-gifts-editor">
      ...
      <div className="profile-gifts-editor__layout"><aside className="profile-studio-live-summary"><span className="profile-kicker"><Gift size={14} /> Brouillon</span><div className="profile-gifts-summary-object"><GiftObject item={giftItemFor(gift)} /><span><small>{giftItemFor(gift).rarity}</small><em>{gift ? !giftInventoryActive ? "Activation en préparation" : selectedGiftAvailability === null ? "Stock non chargé" : `${selectedGiftAvailability} disponible${selectedGiftAvailability > 1 ? "s" : ""}` : "À composer"}</em></span></div><h4>{gift ?? "Cadeau à choisir"}</h4><p>{recipient || "Destinataire à choisir"}</p><dl><div><dt>Source</dt><dd>{recipientSource}</dd></div><div><dt>État</dt><dd>Brouillon local</dd></div><div><dt>Room prévue</dt><dd>{round}</dd></div></dl><span className="profile-studio-safe-note"><LockKeyhole size={14} /> Aucune distribution déclenchée</span></aside><div className="profile-gifts-editor__canvas"><header><div><small>Étape {step + 1} sur 3</small><h4>{step === 0 ? "Choisir un cadeau" : step === 1 ? "Choisir le destinataire prévu" : "Choisir la Room prévue"}</h4></div><strong>0{step + 1} / 03</strong></header>
        ...
        <footer className="profile-gifts-editor__footer"><button type="button" onClick={() => step === 0 ? setScreen("overview") : setStep((current) => current - 1)}><ArrowLeft size={15} /> {step === 0 ? "Annuler" : "Retour"}</button><button type="button" className="is-primary" onClick={() => { if (step === 0 && !gift) { setFormMessage("Choisis un cadeau"); return; } if (step < 2) setStep((current) => current + 1); else saveGift(); }}>{step === 2 ? <LockKeyhole size={16} /> : null}{step === 2 ? "Enregistrer le brouillon" : "Continuer"}{step < 2 ? <ArrowRight size={15} /> : null}</button></footer>
      </div></div>
    </div>}
