...
export default function ProfilePrivateModuleView({
  ...
  const scrollToMobileRecord = (selector: string) => {
    if (!window.matchMedia("(max-width: 767px)").matches) return;
    window.requestAnimationFrame(() => {
      const target = document.querySelector<HTMLElement>(`.profile-space-view ${selector}`);
      if (!target) return;
      target.focus({ preventScroll: true });
      target.scrollIntoView({ block: "start", behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth" });
    });
  };
  const selectMobileRecord = (id: string) => {
    setSelectedId(id);
    scrollToMobileRecord(".profile-private-record-detail");
  };
  const recordBack = <button type="button" className="profile-private-detail-back" onClick={() => scrollToMobileRecord(".profile-private-record-list")}><ArrowLeft size={15} /> Retour à la liste</button>;
  ...
  const renderContractsWorkspace = () => (
    <section className="profile-private-contracts-workspace" aria-label="Contrats et aperçu documentaire">
      <aside className="profile-private-contract-list profile-private-record-list" tabIndex={-1}>
        ...
        <div>{visibleRecords.map((record) => <button key={record.id} type="button" className={record.id === selected?.id ? "is-active" : ""} onClick={() => selectMobileRecord(record.id)}><span><record.icon size={17} /></span><span><strong>{record.title}</strong><small>{record.subtitle}</small><em>{record.meta}</em></span><i>{resolvedIds.includes(record.id) ? "Traité" : record.status}</i><ChevronRight size={14} /></button>)}</div>
      </aside>
      <article className="profile-private-contract-detail profile-private-record-detail" tabIndex={-1}>{recordBack}
  ...
  const renderHardwareWorkspace = () => (
    <section className="profile-private-equipment-workspace" aria-label="Inventaire du matériel">
      ...
      <aside className="profile-private-equipment-list profile-private-record-list" tabIndex={-1}>{visibleRecords.map((record) => <button key={record.id} type="button" className={record.id === selected?.id ? "is-active" : ""} onClick={() => selectMobileRecord(record.id)}><span><record.icon size={18} /></span><span><strong>{record.title}</strong><small>{record.subtitle}</small></span><i>{record.status}</i><ChevronRight size={14} /></button>)}</aside>
      <article className="profile-private-equipment-detail profile-private-record-detail" tabIndex={-1}>{recordBack}{selected && <><div className="profile-private-equipment-detail__hero"><span><SelectedIcon size={25} /></span><div><small>Équipement sélectionné</small><h3>{selected.title}</h3><p>{selected.description}</p></div><em>{selected.status}</em></div><div className="profile-private-detail__facts">{selected.facts.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div><div className="profile-private-availability"><div><span className="profile-kicker"><CalendarDays size={13} /> Disponibilités</span><h4>{allowDemoActions ? "Semaine du 13 juillet" : "Planning non câblé"}</h4></div>{allowDemoActions ? <div>{["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"].map((day, index) => <span key={day} className={index === 4 || selected.status === "À vérifier" ? "is-limited" : "is-open"}><strong>{day}</strong><small>{index === 4 || selected.status === "À vérifier" ? "Indispo." : "18–23 h"}</small></span>)}</div> : <p>Le schéma actuel ne contient pas encore de créneaux de disponibilité.</p>}</div><div className="profile-private-detail__actions"><button type="button" className="profile-primary-button" disabled={!allowDemoActions} onClick={() => allowDemoActions && onOpenQuickAction({ kind: "edit-hardware", equipmentName: selected.title })}><PencilLine size={15} /> Modifier</button><button type="button" className="profile-soft-button" disabled={!allowDemoActions} onClick={() => allowDemoActions && onOpenQuickAction({ kind: "hardware-unavailability", equipmentName: selected.title })}>Signaler une indisponibilité</button></div></>}
      </article>
    </section>
  );
