...
function PlaceRoomExperienceContent({ requestedRoomId, currentUserId, demoRole, demoRoom, onOpenProfile, onMessageProfile, onCollaborateProfile, onLeaveRoom, onLiveCallRequest }: PlaceRoomExperienceProps) {
  ...
  const roomMedia = usePlaceLiveKitRoom(room.id, {
    enabled: !place.isLoading && room.source === "live" && room.status === "live",
  ...
  const programAudio = useMemo(() => ({
    ...
    musicAudible: rtcMediaMatchesRoom && roomMedia.musicAudible,
    musicGeneration: rtcMediaMatchesRoom ? roomMedia.musicGeneration : null,
    prepareMusicTrack: roomMedia.prepareMusicTrack,
    setMusicEnabled: roomMedia.setMusicEnabled,
    releaseMusicTrack: roomMedia.releaseMusicTrack,
  }), [rtcMediaMatchesRoom, roomMedia.musicAudible, roomMedia.musicGeneration, roomMedia.prepareMusicTrack, roomMedia.releaseMusicTrack, roomMedia.setMusicEnabled, roomMedia.status]);
  ...
  const localAudio = usePlaceLocalAudio(room.personalVocal, {
    ...
    pitchAdapter: room.personalVocal.tuneEnabled
  ...
  useEffect(() => {
    if (room.source !== "live" || room.status !== "live" || !roomMedia.canPublish) return;
    let current = true;
    ...
    return () => { current = false; };
  }, [hostCameraEnabled, place.setOwnCameraEnabled, room.source, room.status, roomMedia.canPublish, roomMedia.setCameraEnabled, showNotice]);
    ...
    <RoomPresentationProvider presentation={roomPresentation}>
      ...
      {place.isLoading ? (
        ...
        <PlaceStudioPanel
          ...
          onAudioPreview={place.prepareTrackPreview}
          onAudioPreviewMetadata={place.updateTrackPreviewMetadata}
          onAudioRoute={place.setTrackRoute}
          onAudioPlaybackState={place.setTrackPlaybackState}
