...
export default function RoomSupportPanel({ hostName, wallet, canEngage, onClose, prepared, onPrepare, onCancelPrepared }: Props) {
  ...
  return <aside ref={panel} className="place-donation-drawer room-support-panel" role="dialog" aria-modal="false" aria-labelledby="place-donation-title" tabIndex={-1}
    ...
    {view === "funds" ? <>
      ...
      {prepared && <p className="room-support-panel__hint">{prepared.amounts.length - prepared.sent} lancer(s) encore prêts dans la navbar. Tu peux les remplacer ou les annuler ici.</p>}
      ...
      <p className="room-support-panel__hint">Ensuite, clique sur la petite bourse de la navbar pour chaque lancer.</p>
