  ...
  return (<div className="classroom-roster">
      ...
      {seats.map((seat) => {
        ...
        >
          ...
          </button> : onSelectFreeSeat ? <button type="button" className="classroom-seat__empty" aria-label={`Acheter la place ${seat.number}`} onClick={() => onSelectFreeSeat(seat.number)}><b className="classroom-seat__number">{String(seat.number).padStart(2, "0")}</b><span><Armchair /></span><small>{((classe.seatPriceCents ?? 499) / 100).toLocaleString("fr-FR", {style:"currency",currency:"EUR"})}</small></button> : <div className="classroom-seat__empty"><b className="classroom-seat__number">{String(seat.number).padStart(2, "0")}</b><span><Armchair /></span><small>Libre</small></div>}
        </article>;
      })}
      </div>
    </div>);
