...
export class PlaceLocalAudioEngine {
  ...
  async setPitchAdapter(adapter: PlacePitchCorrectionAdapter | null) {
    ...
    if (nextProcessor) {
      try {
        ...
        this.pitchHealthUnsubscribe = nextProcessor.subscribeHealth?.(() => this.emit()) ?? null;
      } catch (error) {
  ...
  private async createGraph(token: number) {
    ...
    try {
      ...
      this.pitchHealthUnsubscribe = pitchProcessor?.subscribeHealth?.(() => this.emit()) ?? null;
  ...
  private applySettings() {
    ...
    const reverbAmount = this.settings.reverbEnabled ? clamp(this.settings.reverbAmount, 0, 1) : 0;
