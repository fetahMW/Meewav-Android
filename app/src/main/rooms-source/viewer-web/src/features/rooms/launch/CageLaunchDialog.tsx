...
type Props = { onClose: () => void; closeRef: RefObject<HTMLButtonElement | null>; fromProfile?: boolean };
const rosterLabels = {
...
export default function CageLaunchDialog({ onClose, closeRef, fromProfile = false }: Props) {
      ...
      ? !configuration.title.trim()
        ...
        : configuration.title.trim().length > 100
          ...
          : !Number.isInteger(configuration.participantCount) || configuration.participantCount < (configuration.format === "open-mic" ? 1 : 2) || configuration.participantCount > 64
            ...
            : null
      : validateCageLaunch(configuration);
    if (validation) { setError(validation); return; }
    if (step === 1 && configuration.format !== "open-mic" && configuration.rules.votingMode !== "public") {
  ...
  const launch = async () => {
    ...
    try {
      const preview = import.meta.env.DEV && (import.meta.env.VITE_ROOMS_WORKSPACE_PREVIEW === "true" || !user);
      if (preview) {
  ...
  return <section className="rooms-home-launch-dialog__panel cage-launch" role="dialog" aria-modal="true" aria-labelledby="rooms-home-launch-title" aria-describedby="rooms-home-launch-description" onKeyDown={(event) => {
    ...
    <header className="cage-launch__header"><span className="cage-launch__mark"><Trophy /></span><div><span className="cage-launch__eyebrow">SÉQUENCEUR DE LANCEMENT</span><h2 id="rooms-home-launch-title">Préparer La Cage</h2><p id="rooms-home-launch-description">Le format, les participants et les règles accompagnent ton live.</p></div><button ref={closeRef} type="button" className="cage-launch__close" onClick={onClose} disabled={pending} aria-label="Fermer"><X /></button></header>
    <nav className="cage-launch__steps" aria-label="Étapes de lancement">{["Compétition", "Règlement", "Résumé", "Green Room"].map((label, index) => <span key={label} aria-current={step === index ? "step" : undefined} className={step === index ? "is-active" : ""}><i>{step > index ? <Check size={12} /> : index + 1}</i>{label}</span>)}</nav>
