...
function FeedTile({
  ...
  return <article className={`cage-stage-feed is-${side.toLowerCase()}${live ? " is-live" : ""}${winner ? " is-winner" : ""}${portraitPresentation ? " is-portrait-presentation" : ""}`} data-side={side} data-feed={assignment?.exact ? "exact" : assignment ? "demo" : "missing"} data-media-format={aspectRatio ?? "unknown"}>
