...
function VolumeRow({
  ...
  const level = channel.isMuted || meterSuppressed ? 0 : Math.min(1, Math.max(0, channel.level));
  ...
  return (
    <article className={`place-volume-row${isMaster ? " is-master" : ""}${channel.kind === "audio" ? " is-music-channel" : channel.kind === "master" ? " is-master-channel" : " is-voice-channel"} is-${channel.signalState}${level >= 0.82 && channel.signalState !== "clipping" ? " is-near-peak" : ""}${channel.isMuted ? " is-muted" : ""}`} style={style}>
    ...
    <section ref={selectorsRef} className="place-autotune-controls" data-provider={pitchProvider} aria-label="Réglages Autotune">
      ...
      <div className="place-autotune-controls__ranges">
        ...
        <DetailRange
          ...
          lowLabel="Précis"
          highLabel="Humain"
          disabled={!editable || !humanizeSupported}
          onChange={(value) => {
  ...
  return (
    <div className={`place-mixer${mode !== "guest" || personalMode ? " has-audio-player" : ""}${personalMode ? " is-personal-mix" : ""}${showRoomTools ? " is-wave-tools" : ""}${classroomPlayerCollapsible ? " has-classroom-player" : ""}${classroomPlayerCollapsed ? " is-classroom-player-collapsed" : ""}`} aria-label={`Régie audio de ${roomPresentation.label}`}>
      ...
      {activeView === "volumes" ? (
        ...
        <section className="place-fx-view is-accordion">
          <div className="place-fx-accordion-stack">
            <FxAccordion
              id="effects"
              title="Effets voix"
