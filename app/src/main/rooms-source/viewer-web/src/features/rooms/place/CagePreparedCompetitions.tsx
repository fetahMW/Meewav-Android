...
export default function CagePreparedCompetitions() {
  ...
  return <>
    ...
    {open && createPortal(<div style={bounds} className={`cage-prepared-overlay rooms-home-launch-dialog${closing ? " is-closing" : ""}`} onMouseDown={(event) => { if (event.target === event.currentTarget) closeRef.current?.click(); }} onKeyDown={(event) => {
