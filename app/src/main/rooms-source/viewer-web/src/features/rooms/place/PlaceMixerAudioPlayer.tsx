...
export default function PlaceMixerAudioPlayer({
  ...
  const toggleLoopRegion = () => {
    if (waveEngine && waveState) { waveEngine.setRegion(cueSeconds, waveState.bars, !waveState.region); return; }
  ...
  const moveLoopEdge = (edge: "start" | "end", seconds: number) => {
    if (waveEngine && waveState?.region) { waveEngine.setRegion(edge === "start" ? seconds : seconds - (waveState.region.end - waveState.region.start), waveState.bars); return; }
    const region = loopRegionRef.current;
    if (!region) return;
  ...
  return (
    <section
      className={`place-mixer-audio${showWaveListeningSelector ? " has-wave-listening" : ""}${currentTrack ? " has-track" : " is-empty"}${route === "public" ? " is-public-route" : " is-preview-route"}${publicAudienceConfirmed ? " is-public-playing" : ""}${pickerView === "sources" ? " has-source-menu" : ""}${classroomCollapsible ? " is-classroom-collapsible" : ""}${classroomCollapsible && classroomCollapsed ? " is-classroom-collapsed" : ""}`}
    ...
    >
      ...
      >
        <div className="place-mixer-audio__body">
          <div className="place-mixer-audio__track">
            <span className="place-mixer-audio__track-summary">
              <span className="place-mixer-audio__track-icon" aria-hidden="true"><Music2 /></span>
              <span className="place-mixer-audio__meta">
                ...
                {trackHint ? <span className={`place-mixer-audio__track-hint${waveState?.quickPreview ? " is-quick-preview" : ""}`}>{trackHint}</span> : null}
              </span>
            </span>
            {showWaveListeningSelector && waveState && waveEngine ? <WaveListeningSelector mode={waveState.quickPreview ? "loop" : waveState.mode} hasCandidate={Boolean(waveState.candidate)}
              onChange={(mode) => waveEngine.setMode(mode)} /> : null}
            <button
              type="button"
          ...
          >
            ...
            {currentTrack ? <span className="place-mixer-audio__rail-label is-playback" aria-hidden="true">Lecture</span> : null}
            {loopRegion ? <PlaceMixerLoopRange region={loopRegion} duration={durationSeconds} onChange={moveLoopEdge} musical={waveState ? { grid: waveState.grid, bars: waveState.bars } : undefined} /> : <input
      ...
      {(pickerView === "library" || pickerView === "setlists" || pickerView === "queue") && typeof document !== "undefined" ? createPortal(
        <div
