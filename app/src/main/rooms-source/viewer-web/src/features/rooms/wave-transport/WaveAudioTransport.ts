...
export type WaveTransportSnapshot = {
  ...
  playing: boolean; position: number; duration: number; grid: WaveGrid; bars: WaveBars; region: WaveRegion | null;
...
export class WaveAudioTransport {
  ...
  private state: WaveTransportSnapshot = { referencePeaks: [], referencePeriod: 0, referenceId: null, referenceLoading: false, playing: false, position: 0, duration: 0,
    grid: { bpm: 0, beatsPerBar: 4, origin: 0 }, bars: 4, region: null,
  ...
  setGrid(grid: WaveGrid) {
    ...
    if (this.state.region) this.setRegion(this.state.region.start, this.state.bars);
  ...
  setRegion(start: number, bars: WaveBars, enabled = true) {
    ...
    this.patch({ bars, region, duration: Math.max((this.reference ? this.reference.period / this.reference.rate : 0), region?.end ?? 0) });
