...
export default function PlaceBackstageControls({ room, participant, isHost, liveKitVideoTracks = [] }: {
  room: PlaceRoomState; participant: PlaceParticipant; isHost: boolean; liveKitVideoTracks?: PlaceLiveKitVideoTrack[];
}) {
  ...
  useEffect(() => { if (!isHost || participant.status !== "backstage") { setOpen(false); dialog.current?.close(); } }, [isHost, participant.status]);
  return <div className="place-guest-media-controls" role="group" aria-label={`Actions privées pour ${name}`}>
