      ...
      <div ref={menuRef} id={id} className="place-mixer-playback-popover" role="menu" aria-label="Mode de lecture"
        ...
        {transport && wave ? <>
          ...
          <div className="wave-musical-length" role="group" aria-label="Longueur de la zone A/B">
            {([4, 8, 16] as const).map(bars => <button type="button" role="menuitemradio" key={bars}
              tabIndex={-1} aria-label={`${bars} mesures`} aria-checked={wave.bars === bars}
        ...
        <p id={`${id}-loop-hint`} className="place-mixer-playback-popover__hint">
          {canLoop && wave ? "Chevrons et glisser : pas de 4 mesures, longueur conservée." : canLoop ? "Glissez A et B pour régler la zone. La cible revient au début." : "Chargez un son pour définir une boucle A–B."}
        </p>
