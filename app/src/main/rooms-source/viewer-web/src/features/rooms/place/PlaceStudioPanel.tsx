...
export function ParticipantRow({ participant, variant, profileSource = "live", actions, statusText, timeNow, isJuror = false, selected = false, onSelectedChange, onRemove, onOpenProfile, onMessageProfile, onCollaborateProfile, stageControls }: {
  participant: PlaceParticipant;
  ...
  return (
    <article className={`place-guest-row is-${variant}${isJuror ? " is-juror" : ""}${isGreenHouse ? " is-green-house" : ""}${hasUnstableConnection ? " is-connecting" : ""}${onSelectedChange ? " is-selectable" : ""}${selected ? " is-selected" : ""}`}>
...
function PlaceChatWorkspace({
  ...
  return (
    <div className="place-chat-workspace is-wave-tool-skin" data-active-tool={activeTool}>
...
function PlaceGuests({
  ...
  useEffect(() => { setJuryOnly(false); setJuryError(""); }, [room.id]);
  useEffect(() => {
  ...
  }, [backstage, room.queue]);

  return (
    <div className="place-guests">
      <header className="place-guests__segments" role="tablist" aria-label="Gestion des invités">
        <button type="button" className={`is-queue${section === "queue" ? " is-active" : ""}`} onClick={() => { setSection("queue"); closeFilters(); }} role="tab" aria-selected={section === "queue"} aria-controls="place-guests-queue" aria-label={`File d’attente, ${waitingCount} invités en attente`} title={`File d’attente · ${waitingCount} invités en attente${greenHouseCount ? ` · ${greenHouseCount} en préparation` : ""}`}>
          <Clock3 aria-hidden="true" />
          <span className="place-guests__segment-copy"><small>Attente</small><strong>{waitingCount}</strong></span>
        </button>
        <button type="button" className={`is-backstage${section === "backstage" ? " is-active" : ""}`} onClick={() => { setSection("backstage"); closeFilters(); }} role="tab" aria-selected={section === "backstage"} aria-controls="place-guests-backstage" aria-label={`Coulisses, ${backstage.length} invités prêts`} title={`Coulisses · ${backstage.length} invités prêts`}>
          <UsersRound aria-hidden="true" />
          <span className="place-guests__segment-copy"><small>Coulisses</small><strong>{backstage.length}</strong></span>
        </button>
        <button type="button" className={`is-stage${section === "stage" ? " is-active" : ""}`} onClick={() => { setSection("stage"); closeFilters(); }} role="tab" aria-selected={section === "stage"} aria-controls="place-guests-stage" aria-label={`Sur scène, ${onStage.length} invités sur 3`} title={`Sur scène · ${onStage.length} invités sur 3`}>
      ...
      <section id="place-guests-stage" className="place-guests__list is-onstage" role="tabpanel" hidden={section !== "stage"}>
        ...
        <div className="place-guests__rows">
          {filteredStage.length > 0 ? filteredStage.map((participant) => <ParticipantRow profileSource={room.source} participant={participant} variant="onstage" stageControls={<PlaceGuestMediaControls participant={participant} room={room} {...mediaControls} />} actions={[{ direction: "down", tone: "amber", label: "Envoyer en Coulisses", text: "Vers les coulisses", onAction: () => void onMoveGuest(participant, "backstage") }]} statusText={`En scène depuis ${waitingTime(participant.joinedAt, timeNow)}`} onRemove={() => void onRemoveGuest(participant)} onOpenProfile={() => onOpenProfile(participant.profile.id)} onMessageProfile={() => onMessageProfile(participant.profile.id)} onCollaborateProfile={() => onCollaborateProfile(participant.profile)} key={participant.id} />) : <p className="place-guests__empty"><strong>{onStage.length ? "Aucun profil avec ces filtres" : "Scène libre"}</strong><span>{onStage.length ? "Modifie les filtres pour retrouver les artistes en scène." : "Monte jusqu’à trois invités."}</span></p>}
        </div>
      </section>
      <section id="place-guests-backstage" className="place-guests__list is-backstage" role="tabpanel" hidden={section !== "backstage"}>
        ...
        <div className="place-guests__rows">
          {filteredBackstage.length > 0 ? filteredBackstage.map((participant) => <ParticipantRow profileSource={room.source} participant={participant} variant="backstage" isJuror={juryPolicy.jurorIds.includes(participant.profile.id)} stageControls={<PlaceBackstageControls participant={participant} room={room} {...mediaControls} />} actions={[
      ...
      <section id="place-guests-queue" className="place-guests__list is-queue" role="tabpanel" hidden={section !== "queue"}>
        ...
        {filteredQueue.length > 0 ? filteredQueue.map((participant) => {
          ...
          return <ParticipantRow profileSource={room.source} participant={participant} variant="queue" actions={actions} statusText={statusText} timeNow={timeNow} selected={momentVipPicker ? isMomentVipGuest : selectedQueueIds.includes(participant.id)} onSelectedChange={!momentVipPicker && isRawRequest ? (selected) => toggleQueueSelection(participant.id, selected) : undefined} onRemove={() => void onRemoveGuest(participant)} onOpenProfile={() => onOpenProfile(participant.profile.id)} onMessageProfile={() => onMessageProfile(participant.profile.id)} onCollaborateProfile={() => onCollaborateProfile(participant.profile)} key={participant.id} />;
