...
export default function RoomArtistProfileContent({artist,onOffer}:{artist:PreProfileDemoArtist;demo:boolean;onOffer:()=>void;onOpenProfile?:(id:string)=>void;onContact?:(id:string)=>void}) {
 ...
 return <div ref={container} className="cage-profile-content"><div className="ring-artist-preprofile"><ArtistProfileCard selection={selection} onClose={()=>{}}/>{footer&&createPortal(<button type="button" className="guest-offer" aria-label={`Offrir un cadeau à ${artist.name}`} onClick={onOffer}><Gift/><span>Offrir</span></button>,footer)}</div></div>;
}
