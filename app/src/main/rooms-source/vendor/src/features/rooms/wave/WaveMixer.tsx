...
function ChannelStrip({
  ...
  const meterHeight = channel.muted ? 0 : Math.min(1, level * 1.6) * 100;

  return (
    <div className={`wvm-strip${channel.muted ? " is-muted" : ""}`}>
      <span className="wvm-strip-label">{channel.label}</span>
      <div className="wvm-strip-body">
        <div className="wvm-meter" aria-hidden>
          <div className="wvm-meter-fill" style={{ height: `${meterHeight}%` }} />
        </div>
        <div
          ref={faderRef}
          className="wvm-fader"
      ...
      <button
        ...
        onClick={onMute}
        aria-label={channel.muted ? `Réactiver ${channel.label}` : `Muter ${channel.label}`}
      >
        {channel.hostForcedMuted ? "MUÉ HOST" : channel.muted ? "MUÉ" : "MUTE"}
      </button>
    </div>
  );
}
...
function DeckPlayer({ baseSrc, baseTitle }: { baseSrc: string; baseTitle: string }) {
  ...
  return (
    <div className="wvm-deck">
      <div className="wvm-deck-head">
        <span className="wvm-deck-kicker">PISTE</span>
        <span className="wvm-deck-title">{baseTitle} · Version active</span>
        <button
          className={`wvm-deck-loop${loopOn ? " is-on" : ""}`}
          onClick={() => {
            const next = !loopOn;
            setLoopOn(next);
      ...
      <div className="wvm-deck-wave" onPointerDown={seekAt}>
        {loading ? (
      ...
      <div className="wvm-deck-foot">
        <button className="wvm-deck-play" onClick={toggle} disabled={loading}>
          {loading ? <Loader2 size={14} className="wv-spin" /> : playing ? <Square size={13} fill="currentColor" /> : <Play size={14} fill="currentColor" />}
        </button>
...
export default function WaveMixer({
  ...
  return (
    <div className="wvm">
      <header className="wvm-head">
        <div className="wvm-head-left">
          <span className="wvm-head-title">MIXER</span>
          <span className="wvm-head-ctx">
            <span className="wvm-head-dot" />
            {isViewer ? "INVITÉ" : "HOST"}
          </span>
        </div>
        <div className="wvm-head-dots">
          {[0, 1].map((i) => (
            <button
              key={i}
        ...
        <div className="wvm-head-right">
          ...
          <button
            className="wvm-util wvm-util--preset"
            onClick={() => setPresetPicker(true)}
          >
            {VOCAL_PRESETS.find((p) => p.id === preset)?.label.toUpperCase()}
          </button>
        </div>
      </header>
          ...
          <div className="wvm-treatment">
            ...
            <div className="wvm-fx">
              ...
              <div className="wvm-fx-head">
                <span className="wvm-fx-title">FX VOIX</span>
              ...
              {mode === "simple" ? (
                <div className="wvm-fx-list">
                  <EffectCard kind="tune" enabled={enabledFx.has("tune")} onToggle={() => toggleFx("tune")} onSettings={() => setEditingFx("tune")} />
                  <EffectCard kind="reverb" enabled={enabledFx.has("reverb")} onToggle={() => toggleFx("reverb")} onSettings={() => setEditingFx("reverb")} />
                </div>
              ) : (
                <div className="wvm-fx-list">
