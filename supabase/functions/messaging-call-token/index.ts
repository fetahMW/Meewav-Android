...
Deno.serve(async request => {
  ...
  try {
    ...
    if (typeof callId !== 'string' || !/^[0-9a-f-]{36}$/i.test(callId)) return json({error:'invalid_call'},400);
    ...
    if (error || !call || call.status !== 'accepted') return json({error:'call_not_allowed'},403);
    ...
    const token = await generateBytePlusToken({appId,appKey,roomId:call.roomId,userId:auth.user.id,
      canPublish:true,expiresAt});
    return json({appId,token,identity:auth.user.id,roomId:call.roomId,peerId:call.peerId,expiresAt});
