...
const source = join(root, 'app/src/main/tremplin-source');
const vendor = join(source, 'vendor');
const output = join(root, 'app/src/main/assets/tremplin');
...
const result = await build({
  ...
  outdir: join(output, 'assets'), publicPath: '/tremplin/assets', entryNames: 'main', chunkNames: '[name]-[hash]',
...
let notices = 'Bibliothèques du Tremplin Android, reprises de la chaîne Web.\n';
...
await mkdir(join(root, 'app/build/reports/tremplin'), { recursive: true });
await writeFile(join(root, 'app/build/reports/tremplin/bundle.json'), JSON.stringify(result.metafile, null, 2));
console.log(`Tremplin bundle: ${Object.keys(assets).length} local assets, ${packages.size} dependency licenses.`);
