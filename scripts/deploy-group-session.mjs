...
try {
 ...
 const sql=await readFile(resolve(root,'supabase/migrations/20260923110000_group_session_edit.sql'),'utf8');
 await client.query(sql.replace(/commit;\s*$/, 'rollback;'));
 console.log('Group session editing validated in rolled-back transaction.');
 if(process.argv.includes('--apply')) { await client.query(sql);await client.query("NOTIFY pgrst, 'reload schema'"); console.log('Group session editing contract deployed.'); }
