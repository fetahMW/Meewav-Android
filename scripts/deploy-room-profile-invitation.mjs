...
try {
 ...
 const sql=await readFile(resolve(root,'supabase/migrations/20260923140000_rooms_profile_invitation.sql'),'utf8');
 await client.query(sql.replace(/commit;\s*$/, 'rollback;'));
 console.log('Room profile invitations validated in rolled-back transaction.');
 if(process.argv.includes('--apply')) { await client.query(sql);await client.query("NOTIFY pgrst, 'reload schema'"); console.log('Room profile invitations contract deployed.'); }
