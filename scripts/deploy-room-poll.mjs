...
try {
 ...
 const sql=await readFile(resolve(root,'supabase/migrations/20260923093000_rooms_poll_compatibility.sql'),'utf8');
 await client.query(sql.replace(/commit;\s*$/, 'rollback;'));
 console.log('Poll compatibility validated in rolled-back transaction.');
 if(process.argv.includes('--apply')) { await client.query(sql);await client.query("NOTIFY pgrst, 'reload schema'"); console.log('Poll compatibility contract deployed.'); }
