...
try {
 ...
 const interaction=await readFile(resolve(root,'supabase/migrations/20260922100000_scene_interactions_v1.sql'),'utf8');
 const likes=await readFile(resolve(root,'supabase/migrations/20260808091000_scene_media_likes.sql'),'utf8');
 const sql=interaction.replace('begin;', () => 'begin;\n'+likes);
 await client.query(sql.replace(/commit;\s*$/, 'rollback;'));
 console.log('Scene migration validated in rolled-back transaction.');
 if(process.argv.includes('--apply')) { await client.query(sql); console.log('Scene interactions contract deployed.'); }
