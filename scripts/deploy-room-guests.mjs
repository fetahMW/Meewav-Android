...
try {
 ...
 const sql=await readFile(resolve(root,'supabase/migrations/20260923100000_rooms_guest_commands.sql'),'utf8');
 await client.query(sql.replace(/commit;\s*$/, 'rollback;'));
 console.log('Guest commands validated in rolled-back transaction.');
 if(process.argv.includes('--apply')) { await client.query(sql);await client.query("NOTIFY pgrst, 'reload schema'"); console.log('Guest commands contract deployed.'); }
