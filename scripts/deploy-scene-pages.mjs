...
try {
 ...
 const sql=await readFile(resolve(root,'supabase/migrations/20260923120000_scene_comments_pagination.sql'),'utf8');
 await client.query(sql.replace(/commit;\s*$/, 'rollback;'));
 console.log('Scene comment pagination validated in rolled-back transaction.');
 if(process.argv.includes('--apply')) { await client.query(sql);await client.query("NOTIFY pgrst, 'reload schema'"); console.log('Scene comment pagination contract deployed.'); }
