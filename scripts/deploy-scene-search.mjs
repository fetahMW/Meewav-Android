...
try {
  ...
  const sql = await readFile(resolve(root, 'supabase/migrations/20260923150000_scene_catalog_search.sql'), 'utf8');
  await client.query('begin');
  await client.query(sql);
  await client.query('rollback');
  console.log('Scene search contract validated in rolled-back transaction.');
  if (process.argv.includes('--apply')) {
    await client.query(sql);
    await client.query("NOTIFY pgrst, 'reload schema'");
    console.log('Scene search contract deployed.');
  }
} catch (error) {
