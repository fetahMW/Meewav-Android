...
try {
 ...
 const sql=await readFile(resolve(root,'supabase/migrations/20260921170000_loge_host_viewer_tools.sql'),'utf8');
 await client.query(sql.replace(/commit;\s*$/, 'rollback;'));
 console.log('Loge migration validated in rolled-back transaction.');
 if(process.argv.includes('--apply')) { await client.query(sql);await client.query("NOTIFY pgrst, 'reload schema'"); console.log('Loge shared contract deployed.'); }
