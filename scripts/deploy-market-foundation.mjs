...
try {
 ...
 const sql='begin;\n'+(await Promise.all(['20260719030000_marketplace_catalog_foundation_v1.sql','20260719040000_marketplace_catalog_filters_v2.sql'].map(file=>readFile(resolve(root,'supabase/migrations',file),'utf8')))).map(s=>s.replace(/^\s*(begin|commit);\s*$/gmi,'')).join('\n')+'\ncommit;';
 await client.query(sql.replace(/commit;\s*$/, 'rollback;'));
 console.log('Marketplace migration validated in rolled-back transaction.');
 if(process.argv.includes('--apply')) { await client.query(sql);await client.query("NOTIFY pgrst, 'reload schema'"); console.log('Marketplace shared contract deployed.'); }
