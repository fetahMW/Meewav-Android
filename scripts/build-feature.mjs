...
if (syncAssets) {
  const publicAssets = new Set(syncMediaOnly ? [] : ['images/market', 'images/scene', 'images/shorts', 'media/scene', 'media/shorts', 'images/profile', 'assets/orbit/founder-puff.png', 'badges', 'ui/images', 'images/preprofile/portraits', 'avatars', 'badges', 'images/grades', 'audio/rooms/wave-test-pack/House_124BPM_A_minor/Loops_8bars',
...
let notices = `Bibliothèques ${title} Android, reprises de la chaîne Web.\n`;
...
await mkdir(join(root, `app/build/reports/${surface}`), { recursive: true });
await writeFile(join(root, `app/build/reports/${surface}/bundle.json`), JSON.stringify(result.metafile, null, 2));
console.log(`${title} bundle: ${Object.keys(assets).length} local assets, ${packages.size} dependency licenses.`);
