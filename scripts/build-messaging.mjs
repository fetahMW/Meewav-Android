...
const source = join(root, 'app/src/main/messaging-source');
const vendor = join(source, 'vendor');
const output = join(root, 'app/src/main/assets/messaging');
...
let notices = 'Bibliothèques de la messagerie Android, reprises de la chaîne Web.\n';
...
await mkdir(join(root, 'app/build/reports/messaging'), { recursive: true });
await writeFile(join(root, 'app/build/reports/messaging/bundle.json'), JSON.stringify(result.metafile, null, 2));
console.log(`Messaging bundle: ${Object.keys(assets).length} local assets, ${packages.size} dependency licenses.`);
