...
try {
  ...
  const {rows}=await client.query(`select pg_get_functiondef(p.oid) as definition from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('rooms_create_poll_v2','rooms_stop_poll_v2')`);
  const {writeFile}=await import('node:fs/promises'); await writeFile(resolve(root,'tools/poll-contract.sql'),rows.map(x=>x.definition).join('\n'));
