...
try {
  ...
  const {rows}=await client.query(`select pg_get_functiondef(p.oid) as definition from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('rooms_kick_user_v2','rooms_set_guest_mic_forced_muted_v2','rooms_remove_from_queue_v2','rooms_leave_queue_v2','rooms_v2_assert_host','rooms_classe_reject_applications_v1','rooms_set_host_mic_forced_muted_v2')`);
  const {writeFile}=await import('node:fs/promises'); await writeFile(resolve(root,'tools/guests-native-contract.sql'),rows.map(x=>x.definition).join('\n'));
  const {rows:checks}=await client.query("select table_name,column_name,data_type from information_schema.columns where table_schema='public' and table_name in ('room_invitations_v2','room_queue_v2','room_mixer_state_v2')"); console.log(checks);
