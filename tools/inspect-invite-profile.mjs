...
try {
  ...
  const {rows}=await client.query(`select pg_get_functiondef(p.oid) as definition from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('search_messageable_profiles_v1','messaging_find_contact_by_username_v1','rooms_accept_invitation_v2','rooms_classe_invite_v1')`);
  const {writeFile}=await import('node:fs/promises'); await writeFile(resolve(root,'tools/invite-profile-contract.sql'),rows.map(x=>x.definition).join('\n'));
