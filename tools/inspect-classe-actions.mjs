...
try {
  ...
  const {rows}=await client.query(`select pg_get_functiondef(p.oid) as definition from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('rooms_classe_profile_projection_v1','rooms_classe_cancel_floor_v1','rooms_classe_toggle_question_like_v1','rooms_classe_grant_floor_v1','rooms_classe_set_hands_open_v1')`);
  const {writeFile}=await import('node:fs/promises'); await writeFile(resolve(root,'tools/classe-actions-contract.sql'),rows.map(x=>x.definition).join('\n'));
