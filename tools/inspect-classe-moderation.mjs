...
try {
  ...
  const {rows}=await client.query(`select pg_get_functiondef(p.oid) as definition from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('rooms_classe_resolve_question_v1','rooms_classe_configure_v1','rooms_classe_moderation_kick_v1')`);
  const {writeFile}=await import('node:fs/promises'); await writeFile(resolve(root,'tools/classe-moderation-contract.sql'),rows.map(x=>x.definition).join('\n'));
