...
try {
  ...
  const {rows}=await client.query(`select pg_get_functiondef(p.oid) as definition from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('rooms_classe_set_stage_v1','rooms_classe_accept_application_v1','rooms_invite_from_queue_v2','rooms_move_invitation_to_stage_v2','rooms_move_invitation_to_backstage_v2','rooms_kick_invitation_v2','rooms_cancel_invitation_v2')`);
  const {writeFile}=await import('node:fs/promises'); await writeFile(resolve(root,'tools/guest-transition-contract.sql'),rows.map(x=>x.definition).join('\n'));
