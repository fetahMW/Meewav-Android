...
try {
  ...
  const {rows}=await client.query(`select pg_get_functiondef(p.oid) as definition from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('rooms_classe_viewer_state_v1','rooms_classe_host_state_v1','rooms_classe_request_floor_v1','rooms_classe_submit_question_v1','rooms_classe_release_floor_v1')`);
  const {writeFile}=await import('node:fs/promises'); await writeFile(resolve(root,'tools/classe-tools-contract.sql'),rows.map(x=>x.definition).join('\n'));
