...
try {
  ...
  const {rows}=await client.query(`select pg_get_functiondef(p.oid) as definition from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('rooms_send_message_v2','rooms_classe_send_message_v1','rooms_pin_message_item_v2','rooms_delete_message_v2','rooms_poll_state_v1','list_artist_group_tools_v1','mutate_artist_group_tool_v1')`);
  const {writeFile}=await import('node:fs/promises'); await writeFile(resolve(root,'tools/chat-group-contract.sql'),rows.map(x=>x.definition).join('\n'));
